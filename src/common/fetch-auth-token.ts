...
export async function fetchToken(params: { refreshToken?: string } = {}): Promise<string | null> {
	...
	try {
		...
		if (response.status === 200 && response.data) {
			...
			util.log.error(`${response.status} Failed to refresh token. ${response.statusText}`);
