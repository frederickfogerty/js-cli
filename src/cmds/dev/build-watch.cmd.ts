...
export const description =
	'Starts TypeScript/Babel build-watchers for all modules.';

const TS_MODULES = constants.MODULE_DIRS
	.toPackageObjects()
	.filter(pkg => startsWithOrgName(pkg.name))
	.filter(pkg => fs.existsSync(fsPath.join(pkg.path, 'tsconfig.json')));

const hasBuildWatch = (pkg: constants.IPackageObject) =>
	pkg.hasScript(config.packageScripts.BUILD_WATCH_COMMAND);

export function cmd(args: { params: string[] }) {
	printTitle('TypeScript Build Watchers');

	const modules = getModulesFromParams(
		args.params,
		TS_MODULES.map(pkg => pkg.path),
	)
		.toPackageObjects()
		.filter(pkg => startsWithOrgName(pkg.name))
		.filter(pkg => fs.existsSync(fsPath.join(pkg.path, 'tsconfig.json')));
	...
	const modulesWithoutBuildWatch = modules.filter(pkg => !hasBuildWatch(pkg));
	...
	if (modulesWithoutBuildWatch.length > 0) {
		log.info.gray(
			`Not watching: (no ${config.packageScripts
				.BUILD_WATCH_COMMAND} script found)`,
		);
...
function watchTypescript(
	...
	if (!pkg.hasScript(config.packageScripts.BUILD_WATCH_COMMAND)) {
		return;
	}

	const cmd = `cd ${pkg.path} && yarn run ${config.packageScripts
		.BUILD_WATCH_COMMAND}`;
	execaCommand(cmd).forEach((e: string) => {
