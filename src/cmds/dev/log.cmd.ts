...
export const args = {
	'--dev, -d':
		'Flag indicating if the development log should be examine (default: production).',
};
...
export function cmd(args?: {
	...
	printTitle(
		`Desktop App Log (${log.magenta(dev ? 'development' : 'production')})`,
	);
	...
	log.info();
	run.exec$(cmd).forEach(e => log.info(e.stdout));
}
