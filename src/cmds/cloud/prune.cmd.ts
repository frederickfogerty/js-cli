...
export const description =
	'Deletes unused instances that deployed in the cloud.';
export const args = {
...
export async function cmdDisable(args: {
	...
	// Filters.
	const hasAlias = (id: string) =>
		R.find(R.propEq('deploymentId', id), aliases) !== undefined;
	...
	if (!isTest) {
		if (tasks.length > 0) {
			const confirmed = await prompt.confirm(
				`Delete ${tasks.length} ${pluralize('deployment', tasks.length)}?`,
			);
			if (confirmed) {
